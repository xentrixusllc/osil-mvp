...
def _posture_signal_text(bvsi: float, posture: str) -> str:
    if bvsi >= 85:
        return f"Operational stability is currently <b>{posture}</b>, with a BVSI™ score of {bvsi:.1f} indicating strong control across core stability dimensions."
    if bvsi >= 70:
        return f"Operational stability is currently <b>{posture}</b>, with a BVSI™ score of {bvsi:.1f} indicating improving control with localized areas of exposure."
    if bvsi >= 55:
        return f"Operational stability is currently <b>{posture}</b>, with a BVSI™ score of {bvsi:.1f} indicating that governance mechanisms are functioning but recurring instability patterns remain across higher-impact services."
    if bvsi >= 40:
        return f"Operational stability is currently <b>{posture}</b>, with a BVSI™ score of {bvsi:.1f} indicating visible operational exposure and elevated execution risk."
    return f"Operational stability is currently <b>{posture}</b>, with a BVSI™ score of {bvsi:.1f} indicating material operational fragility and immediate need for stabilization."


def _top_three_briefs(sip_candidates: pd.DataFrame) -> pd.DataFrame:
    df = _safe_df(sip_candidates)
    if df.empty:
        return pd.DataFrame(columns=["Initiative", "Why It Matters"])
...
def build_osil_pdf_report(payload: Dict[str, Any]) -> bytes:
    ...
    story.append(_header_band("Operational Stability Snapshot"))
    ...
    signal_text = _posture_signal_text(bvsi, posture)
    story.append(_signal_box("Executive Signal", signal_text))
    story.append(Spacer(1, 10))
    ...
    ds_df = pd.DataFrame(
        {
            ...
            "Score": [round(_safe_float(v), 1) for v in domain_scores.values()],
        }
    )
    story.append(_simple_table(ds_df, font_size=9))
    story.append(Spacer(1, 8))
    story.append(
        Paragraph(
    ...
    story.append(
        Paragraph(
            ...
            "They should be used to guide leadership briefing, ownership alignment, and targeted operational remediation.",
            styles["OSIL_Body"],
        )
    )
    story.append(Spacer(1, 10))

    brief_df = _top_three_briefs(sip_candidates)
    story.append(Paragraph("Top 3 Initiatives to Brief Leadership", styles["OSIL_Section"]))
    story.append(_simple_table(brief_df, font_size=8))
    ...
    story.append(
        Paragraph(
            "Executive view: Service × Stability Risk across recurrence, MTTR drag, reopen churn, and change collision. "
            "This visual helps leadership identify where concentrated instability is likely to create operational exposure.",
            styles["OSIL_Body"],
        )
    )
    story.append(Spacer(1, 10))
